...
public class GET_Published_Date {
 
public int published_year = 0;
 
 public int Published_Year(String title, String author) 
 {
	...
	JSONObject obj_JSONObject_created_date = null;
     try {
	  obj_JSONObject_created_date = obj_JSONObject_sixth.getJSONObject("published-print");
	} catch (JSONException e) {
		//e.printStackTrace();
	}
    ...
    // System.out.println(obj_JSONObject_created_date);
     ...
     JSONArray date_part_array = null;
	try {
		date_part_array = obj_JSONObject_created_date.getJSONArray("date-parts");
	} catch (JSONException e) {
		//e.printStackTrace();
	}
     JSONArray zero_index_date_part_array = null;
	try {
		zero_index_date_part_array = date_part_array.getJSONArray(0);
	} catch (JSONException e) {
		//e.printStackTrace();
	}
     try {
...
//	e.printStackTrace();
    ...
    try {
		published_year = Integer.parseInt(zero_index_date_part_array.get(0).toString());
